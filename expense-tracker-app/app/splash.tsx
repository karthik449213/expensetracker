...
export const SplashScreen: React.FC = () => {
  ...
  const titleAnimRef = useRef<any>(null);
  const bootAnimRef = useRef<any>(null);
  const glitchAnimRef = useRef<any>(null);

  useFocusEffect(
    React.useCallback(() => {
      // Fade in title
      titleAnimRef.current = Animated.timing(titleOpacity, {
        toValue: 1,
        duration: 800,
      ...
      const bootTimeout = setTimeout(() => {
        bootAnimRef.current = Animated.timing(bootOpacity, {
          toValue: 1,
          duration: 500,
          useNativeDriver: false,
        });
        bootAnimRef.current.start();
      }, 800);

      // Glitch effect
      glitchAnimRef.current = Animated.sequence([
      ...
      return () => {
        ...
        bootAnimRef.current?.stop();
        glitchAnimRef.current?.stop();
        clearTimeout(bootTimeout);
        clearTimeout(timer);
        titleOpacity.setValue(0);
        bootOpacity.setValue(0);
        glitchScale.setValue(1);
      };
